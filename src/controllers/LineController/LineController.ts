...
import { createLineSchemaValidator, updateLineSchemaValidator } from '../../validators/lineValidator';
...
class LineController {
  ...
  async store(request: Request, response: Response) {
    ...
    try {
      await this.createLineUseCase.execute({name, stops});
  ...
  async update(request: Request, response: Response) {
    ...
    try{
        await this.updateLineUseCase.execute({name, id: Number(lineId)});
      return response.json({message: "success"});
    } catch(error) {
