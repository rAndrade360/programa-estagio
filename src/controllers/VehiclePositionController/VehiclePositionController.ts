...
import { createPositionSchemaValidator, updatePositionSchemaValidator } from '../../validators/positionValidator';
...
class VehiclePositionController {
  ...
  async store(request: Request, response: Response) {
    ...
    if( !(await createPositionSchemaValidator({vehicle_id, latitude, longitude})) ) {
  ...
  async getbyId(request: Request, response: Response) {
    ...
    try {
      const vehiclePosition = await this.getOneVehiclePositionUseCase.execute({vehicle_id: Number(vehicleId)});
  ...
  async delete(request: Request, response: Response) {
      const { vehicleId } = request.params;
      try{
        await this.deleteVehiclePositionUseCase.execute({vehicle_id: Number(vehicleId)});
