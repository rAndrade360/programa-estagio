...
import { createStopSchemaValidator, updateStopSchemaValidator } from '../../validators/stopValidator';
...
class StopController {
  ...
  async store(request: Request, response: Response) {
    ...
    if( !(await createStopSchemaValidator({name, latitude, longitude, lines})) ) {
      return response.status(400).json({error: "Exists an incorrect value!"});
    }
    try {    
    await this.createStopUseCase.execute({name, latitude, longitude, lines})
      return response.json({message: 'Stored successfully'});
    } catch (error) {
      return response.status(400).json({error: "Could not Store"});
    }
  }
  ...
  async update(request: Request, response: Response) {
    ...
    const isValid = await updateStopSchemaValidator({name, latitude, longitude})
    if(!isValid) {
      return response.status(400).json({error: "Exists an incorrect value!"});
    }
    try{
