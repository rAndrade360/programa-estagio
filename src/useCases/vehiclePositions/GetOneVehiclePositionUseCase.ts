...
export default class GetOneVehiclePositionUseCase {
    ...
    async execute(position: IGetOneVehiclePositionRequestDTO): Promise<VehiclePosition> {
      const vehiclePosition = await this.vehiclePositionRepository.findById(position.vehicle_id);
